...
function precompute_step() {
    ...
    while(true){
        ...
        var p = Math.floor((cur_j) / trail_length * 100);
        if (p > 100) {
            p = 100;
        }
        if (p > cur_percentage) {
...
function flyingspeed_changed(event) {
    ...
    document.getElementById("rig").setAttribute("movement-controls", "controls: touch; fly: true; acceleration: " + parseFloat(acc)/200);
}
